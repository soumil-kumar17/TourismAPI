...
public class BranchRepository : IBranchRepository
{
    ...
    public async Task<bool> CreateBranch(Branch branch)
    {
        ...
        try
        {
            await _repositoryContext.Branches.InsertOneAsync(branch);
            return true;
    ...
    public async Task<bool> UpdateBranch(Branch branch)
    {
        ...
        try
        {
            var tempBranch = Builders<Branch>.Update.Set(c => c.BranchName, branch.BranchName)
                .Set(c => c.Place, branch.Place).Set(c => c.Website, branch.Website)
                .Set(c => c.Contact, branch.Contact).Set(c => c.Email, branch.Email)
                .Set(c=>c.Package,branch.Package)
                .Set(c => c.UpdatedBy, branch.UpdatedBy)
                .Set(c => c.UpdatedDate, branch.UpdatedDate);
            var result = await _repositoryContext.Branches
                .UpdateOneAsync(c => c.BranchID == branch.BranchID, tempBranch);
